...
function createTask() {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "INSERT INTO tasks (title, description, priority, status, deadline, department, assigned_to, created_by) 
              VALUES (:title, :description, :priority,:status, :deadline, :department, :assigned_to, :created_by)";
      $st = $conn->prepare($sql);
