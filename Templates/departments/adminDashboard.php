...
<div class="container my-5">
  ...
  <p class="text-center">Welcome, <?php echo htmlspecialchars($username); ?>! (Admin)</p>
  ...
  <div class="mb-4 text-center">
    <input type="text" id="dateRange" class="form-control d-inline-block" style="max-width: 300px;" placeholder="Select date range" />
    <button id="filterTasksBtn" class="btn btn-primary ms-2">Filter Tasks</button>
  </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          const calendar = new FullCalendar.Calendar(calendarEl, {
              ...
              eventClick: function(info) {
                ...
                // Call the same function as clicking an edit button
                ...
                .then(task => {
                  if (task && task.id) {
                    ...
                    // Wait for users to load before setting assignee
                    $("#department").on("loadedUsers", function onLoaded() {
                      $("#assigned_to").val(task.assigned_to);
                      $("#department").off("loadedUsers", onLoaded);
                    });

                    // Trigger change to load users
                    $("#department").trigger("change");

                    // Update form state
                    $("#taskFormContainer").data("mode", "edit");
                    $("#formTitle").text("Edit Task");
                    $("#submitBtn").val("Update Task");
                    $("#taskFormContainer").show();

                    $("html, body").animate({
                      scrollTop: $("#taskFormContainer").offset().top
                    }, 500);
                  } else {
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="taskForm" method="post">
              ...
              <input type="hidden" name="department" id="department" value="Finance">
              ...
              <div class="mb-3">
                ...
                <select name="department" id="departmentSelect" class="form-select" required>
                  ...
                  try {
                      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
                      $sql = "SELECT DISTINCT department FROM users WHERE department != '' ORDER BY department ASC";
                      $st = $conn->prepare($sql);
                      $st->execute();
                      $departments = $st->fetchAll(PDO::FETCH_COLUMN);
              ...
              <div class="mb-4">
                <label for="assigned_toSelect" class="form-label">Assign To</label>
                <select name="assigned_to" id="assigned_toSelect" class="form-select" required>
                  <?php
                  $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
                  $sql = "SELECT id, firstName FROM users WHERE department = ''";
                  $st = $conn->prepare($sql);
                  $st->execute();
                  $users = $st->fetchAll();
              ...
              <button type="submit" id="submitBtn" class="btn btn-primary w-100">Create Task</button>
            </form>
          </div>
        </div>
      </div>
...
// Dynamically fetch users based on department
// document.getElementById("department").addEventListener("change", function() {
//     const dept = this.value;
//     const assignedSelect = document.getElementById("assigned_to");
//     assignedSelect.innerHTML = "<option>Loading...</option>";

//     fetch("get_users_by_department.php?department=" + encodeURIComponent(dept))
...
//         })
//         .catch(() => {
//             assignedSelect.innerHTML = "<option>Error loading users</option>";
//         });
// });
...
<script src="templates/include/AdminDashboard.js"></script>
...
<style>
  ...
  /* Optional: Adjust headings or other elements for better visibility on dark background */
  h2, h3, h4, p, label {
    color: #ffdf6c;
  }
</style>
